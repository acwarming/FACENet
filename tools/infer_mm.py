...
class SemSeg:
    ...
    def predict(self, img_fname: str, overlay: bool) -> Tensor:
        if cfg['DATASET']['NAME'] == 'PST900':
            x1 = img_fname.replace('/rgb', '/thermal')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='configs/mfnet.yaml')
    ...
    if test_file.is_file():
        ...
        if cfg['DATASET']['NAME'] == 'PST900':
            ...
            for item in files_:
                name = item.strip()
                file_name0 = os.path.join(test_file, 'rgb')
                file_name1 = os.path.join(file_name0, name)
